    ...
    try {
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            list.forEach(character => {
                characterChooserMenu.innerHTML += `
                <div class='level-card' id='${character.className}'>
                    <h2 class='level-card-title'>${character.name}</h2>
                    <img class='level-card-img' src='${character.imgBase64}' alt='${character.name}'>
                </div>`;
            });
